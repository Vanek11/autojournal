...
const allowedDocumentTypes = Object.values(DocumentType) as DocumentType[];

function parseDocumentType(value: unknown): DocumentType {
  const normalized = String(value || "").trim().toLowerCase();

  const matchedType = allowedDocumentTypes.find(
    (item) => item.toLowerCase() === normalized
  );

  return matchedType || allowedDocumentTypes[0];
}
